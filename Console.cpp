    ...
    while (choice!=0)
    {
        switch (choice)
        {
            ...
            case 2: //选选修课
                ...
                catch(ExceptionStudent a)
                {
                    cout<<"\n"<<a.what();
                    a.print();
                    cout<<"每个学生最多可选"<<MAX_SIZE<<"门课程\n"<<endl;
                }
                break;
	...
	for(int i=0;i<student->getCourseNumber();i++){
		if(typeid(ObligatoryCourse)==typeid((*student)[i])){//利用RTTI,判断是不是必修课
			...
			do
			{
                ...
                cin>>grade;
                try{dynamic_cast<ElectiveCourse &>((*student)[i]).setGrade(grade);}
                catch(ExceptionCourse a)
                {
                    cout<<"\n"<<a.what();
                    a.print();
